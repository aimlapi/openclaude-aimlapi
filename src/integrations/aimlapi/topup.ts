/**
 * AI/ML API seamless top-up flow (DRAFT).
 *
 * End to end:
 ...
 *
 * Everything targets STAGING by default (see config.ts).
 */
...
export type AimlapiTopupOptions = {
  ...
  model?: string
  environment?: AimlapiEnvironment
  partnerId?: string
  partnerName?: string
  inviteCode?: string
...
export async function runAimlapiTopup(options: AimlapiTopupOptions): Promise<void> {
  ...
  const endpoints = resolveEndpoints(environment)
  ...
  console.log(
    chalk.bold(`\n  AI/ML API top-up`) +
      chalk.dim(`  ·  ${environment}  ·  ${endpoints.appBaseUrl}\n`),
  )
  ...
  // 3. Bind + open hosted payment page.
  const { checkout } = await client.pay(token, session.sessionToken, { amountUsdMinor, method })
  if (!checkout.payUrl) {
  ...
  console.log(chalk.green(`\n  ✓ Balance topped up and provider configured.`))
